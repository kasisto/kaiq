...
class IngestionConfig(ProviderConfig):
    ...
    def get_default(cls, mode: str, app) -> "IngestionConfig":
        ...
        if mode == "hi-res":
            return cls(app=app)  # KAIG-492: Removed hardcoded zerox to prevent OOM on large PDFs
        if mode == "ocr":
            return cls(app=app)  # KAIG-492: Removed hardcoded ocr to prevent OOM on large PDFs
        if mode == "fast":
