...
def simple_ingestion_factory(service: IngestionService):
    async def ingest_files(input_data):
        ...
        try:
            ...
            await _ensure_collections_exist(
                document_info, collection_ids
            )
    ...
    async def _ensure_collections_exist(
        ...
        deduplicated_ids = list(set(collection_ids))
        result = await service.providers.database.collections_handler.get_collections_overview(
            offset=0,
            limit=max(len(deduplicated_ids), 100),
            filter_collection_ids=deduplicated_ids,
        )
        ...
        user_collection_ids = set(user_info.collection_ids or [])
        logger.debug("user collection ids: %s", user_collection_ids)
        for collection_id in deduplicated_ids:
            if collection_id in existing_collection_ids:
                if collection_id in user_collection_ids:
                    ...
                    raise R2RException(
                        status_code=403,
                        message=f"Collection {collection_id} does not belong to the document owner.",
                    )
            else:
                # Reject non-existent collection IDs — don't auto-create
                # for arbitrary UUIDs to prevent authorization bypass
                raise R2RException(
                    status_code=404,
